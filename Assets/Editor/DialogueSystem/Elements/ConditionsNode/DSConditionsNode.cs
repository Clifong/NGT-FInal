...
namespace DS.Elements
{
    ...
    public class DSConditionsNode : DSNode
    {
        ...
        public override void Draw()
        {
            ...
            Foldout conditionsFoldout = DSElementUtility.CreateFoldout("Conditions to check");

            VisualElement conditionsListView = CreateConditionsList();
            
            conditionsFoldout.Add(conditionsListView);
            
            customDataContainer.Add(conditionsFoldout);
        ...
        private VisualElement CreateConditionsList()
        {
            ...
            listView.bindItem = (e, i) =>
            {
                ...
                if (Operators[i] != null)
                {
                    objectField.value = Operators[i];
                }
                else
                {
                    Operators[i] = objectField.value as Operator;
                }
            ...
            {
                fieldMapping[Operators.Count - 1].Reset();
                fieldMapping.Remove(Operators.Count - 1);
                view.itemsSource.RemoveAt(Operators.Count - 1);
                view.RefreshItems();
                graphView.CheckIfNodesAreValid();
            };
            
            listView.itemsSource = Operators;
            
            listView.makeItem = () =>
            {
                VisualElement row =  new VisualElement();
                row.style.flexDirection = FlexDirection.Row;
                DSObjectField dsObjectField = new DSObjectField("", typeof(Operator));
                dsObjectField.RegisterValueChangedCallback(evl =>
                {
                    Operators[listViewMap[row]] = evl.newValue as Operator;
        ...
        public override DSNodeSaveData GenerateNodeSaveData(List<DSChoiceSaveData> choices)
        {
            ...
            return new DSConditionsNodeSaveData()
            {
                ID = ID,
                Name = NodeName,
                Choices = Choices,
        ...
        {
            DSConditionsSO conditionsSo;
            
            if (Group != null)
            {
                conditionsSo = DSIOUtility.CreateAsset<DSConditionsSO>($"{containerFolderPath}/Groups/{Group.title}/Conditions", NodeName);
                ...
                dialogueContainer.Groups[createdGroups[Group.ID]].Add(conditionsSo);
            }
            else
            {
                conditionsSo = DSIOUtility.CreateAsset<DSConditionsSO>($"{containerFolderPath}/Global/Conditions", NodeName);

                dialogueContainer.UngroupedNodes.Add(conditionsSo);
            }
            
            conditionsSo.Initialize(
                NodeName,
            ...
            if (IsStartingNode())
            {
                dialogueContainer.AddStartingNode(conditionsSo);
            }
            
            createdNodes.Add(ID, conditionsSo);
            DSIOUtility.SaveAsset(conditionsSo); 
        }
        ...
        public override void AddUngroupedNode<T>(SaintsDictionary<string, T> dictionary, DSGraphView graphView)
        {
            ...
            {
                DSErrorConditionsNodeData nodeErrorData = new DSErrorConditionsNodeData();

                nodeErrorData.Nodes.Add(this);

                (dictionary as SaintsDictionary<string, DSErrorConditionsNodeData>).Add(NodeName, nodeErrorData);
        ...
        public override void RemoveUngroupedNode<T>(SaintsDictionary<string, T> dictionary, DSGraphView graphView)
        {
            ...
            {
                (dictionary as SaintsDictionary<string, DSErrorConditionsNodeData>).Remove(NodeName);
            }
        }
    }
}
